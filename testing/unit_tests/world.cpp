...
TEST_CASE("The refracted color with a refracted ray"){
  World w = World();
  Sphere a = DefaultSphere1();
  a.material.ambient = 1;
  a.material.pattern.active = 1;
  w.addShape(&a);
  Sphere b = DefaultSphere2();
  b.material.transparency = 1;
  b.material.ior = 1.5;
  w.addShape(&b);
  Ray r = Ray(point(0, 0, 0.1), vector(0, 0, 0));
  r.direction = vector(0, 1, 0);
  std::vector<Intersection> xs = {Intersection(-.9899, r, &a), Intersection(-.4899, r, &b), Intersection(.4899, r, &b), Intersection(.9899, r, &a)};
  Tuple c = w.effective_refraction(xs[2], xs[2].generateHitRecord(xs), 5);
...
TEST_CASE("colorAtIntersection with a reflective, transparent material"){
  World w = World();
  Sphere def1 = DefaultSphere1();
  w.addShape(&def1);
  Sphere def2 = DefaultSphere2();
